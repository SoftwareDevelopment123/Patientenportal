...
public class CaseWSTest {
	...
	@Test
	public void getCase() throws MalformedURLException, AuthenticationException, AccessException,
		...
		Assert.assertEquals(compareme.getDoctors().get(0).getSpecialization(),
				pcase.getDoctors().get(0).getSpecialization());
		Assert.assertEquals(compareme.getDoctors().get(0).getUser().getFirstname(),
				pcase.getDoctors().get(0).getUser().getFirstname());
	}

	@Test
	public void getCases() throws MalformedURLException, InvalidParamException, AccessorException, PersistenceException,
		...
		for (Case c : compareme) {
			...
			Assert.assertEquals(c.getPatient().getUser().getFirstname(),
					cases.get(i).getPatient().getUser().getFirstname());
