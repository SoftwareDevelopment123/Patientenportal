...
public class PatientRelativeTest {
	...
	@Test
	public void main() throws MalformedURLException, AuthenticationException, AccessException, AuthorizationException,
		...
		/*
		 * URL url = new URL("http://localhost:8080/relative?wsdl"); QName qname
		 * = new QName("http://services.patientenportal.de/",
		 * "RelativeWSImplService"); Service service = Service.create(url,
		 * qname); RelativeWS rel = service.getPort(RelativeWS.class);
		 */

		// Methode 1
		/*
		 * Accessor accessor = new Accessor(2); Relative relative =
		 * rel.getRelative(accessor);
		 * System.out.println(relative.getRelativeID()); for (Patient p :
		 * relative.getPatients()){ System.out.print("ID: " +
		 * p.getUser().getUserId() + " - ");
		...
		// Methode 2
		/*
		 * Accessor accessor = new Accessor(1); RelativeListResponse response =
		 * rel.getRelativesByP(accessor);
