...
public class DoctorOfficeTest {
	...
	public void main() {
		 ...
		 * OfficeDAO.updateOffice(fulloffice); Assert.assertEquals("Test",
		 * DoctorDAO.getDoctor(2).getSpecialization());
		 */

		// Nachträglicher Userinput-Test (unabhängige Doktor-Entity bis zur
		// Registrierung)
		...
		// Bidirektionaler Zugriff nach nachträglicher Verknüpfung // ab jetzt
		// ausgeschlossen / Transient
		/*
		 * Doctor dx = DoctorDAO.getDoctor(neuD1.getDoctorID()); User ux =
		 * UserDAO.getUser(newuser.getUserId()); Assert.assertEquals("New",
		 * dx.getUser().getFirstname()); Assert.assertEquals("Zahnarzt",
		 * ux.getDoctor().getSpecialization());
		 */

		// DeleteDoctor-Test
		// Info - Rückwärtskaskadierung ist hier nicht eingestellt! Muss auch so
		// sein
		// Doktoren aus der DB zu löschen macht aus Gründen der Datenerhaltung
		// ohnehin keinen Sinn
		// Deswegen muss die Doktor-User-Verknüfung entfernt werden, bevor man
		// den Doktor 1 löschen kann
		...
		// DeleteOffice-Test
		// Info - auch hier keine Kaskadierung vom Office, die Verknüpfung des
		// anderen Doktors zum Office muss/sollte entfernt werden
